...
class ItcSubmitForm {
  ...
  _successXHR(data) {
      ...
      if (key === 'attach') {
        ...
        if (typeof attachs === 'string') {
          ...
          for (let attach in attachs) {
            ...
            const elAttach = this._elForm.querySelector(
              '.form-attach__item[data-index="' + index + '"]',
            );
    ...
    this._elForm
      .querySelectorAll(
        '.form-attach__item:not(.is-invalid), input:not(.is-invalid), textarea:not(.is-invalid)',
      )
      .forEach((el) => {
        this._setStateValidaion(el, 'success', '');
      });
  ...
  _onSubmit() {
    ...
    const submitHeight = this._elForm
      ...
      for (let i = 0, length = target.files.length; i < length; i++) {
        ...
        if (file.type.match(/image.*/)) {
          ...
          reader.addEventListener('load', (e) => {
            this._elForm.querySelector('.form-attach__items').innerHTML +=
              ItcSubmitForm._getAttachTemplate(index, file, e.target);
          });
        } else {
          this._elForm.querySelector('.form-attach__items').innerHTML +=
            ItcSubmitForm._getAttachTemplate(index, file);
        }
      }
      target.value = '';
    });
